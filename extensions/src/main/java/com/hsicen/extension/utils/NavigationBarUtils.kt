 ...
 * 功能：
 * 描述：虚拟导航栏工具类
 * 使用：最好在基类Activity中添加全局回调方法
 * ```
 *  private var mContent: FrameLayout? = null
 *  private var mLayoutComplete = false
 *  private var mNavigationBar = false
 *
 *  onCreate(){
 *      mContent = findViewById(android.R.id.content)
 *      mContent?.post {
 *          mNavigationBar = NavigationBarUtils.navigationBarIsOpen(this)
 *          mLayoutComplete = true
 *      }
 *      mContent?.viewTreeObserver?.addOnGlobalLayoutListener(this)
 *  }
 *
 *  override fun onGlobalLayout() {
 *      if (!mLayoutComplete) return
 *      if (mNavigationBar != NavigationBarUtils.navigationBarIsOpen(this)) {
 *          mNavigationBar = NavigationBarUtils.navigationBarIsOpen(this)
 *          onNavigationBarChange() //方法回调
 *      }
 * }
 *
 * onDestroy(){
